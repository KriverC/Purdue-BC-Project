...
contract Market {
  ...
  function buyItem(uint256 itemId) external payable nonReentrant {
    ...
    IERC721(item.nft).safeTransferFrom(address(this), msg.sender, item.nftId);
    emit Bought(item.nft, msg.sender, item.nftId, item.price);

  }
  
  event Cancel(uint256 listingId);
  
  function cancelSale(uint256 listingId) {
     
    // Check if listing exists in market
    require(listingId < nextId && listingId >= 0, "This listing does not exist currently");

    // Retrieve listing and check if sender is seller
    Listing memory listing = listings[listingId];
    require(message.sender == listing.seller, "You must own this listing to cancel");
